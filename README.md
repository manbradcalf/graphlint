...
Define your graph schema in [SHACL](https://www.w3.org/TR/shacl/). Validate it against any LPG database.
...
It does **not** validate that the schema you provide is itself well-formed or idiomatic. If you hand it a SHACL document with misspelled predicates or unusual patterns, graphlint will silently produce fewer checks rather than reject the input. This is a deliberate tradeoff for a POC — schema authoring validation is a solved problem ([pySHACL](https://github.com/RDFLib/pySHACL) for SHACL), and graphlint assumes your schema has already been validated through those tools or your own review.
...
Early prototype. The core pipeline works: parse SHACL, compile to
